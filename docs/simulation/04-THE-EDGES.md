...
| PE36 | Code older than 15 min | "Sign in" | "That code is wrong or has expired. Ask for a new one." (real time; `age` never moves a future `expires_at`) | lib/patient-auth/code-signin.ts:67, 176-193 |
...
| TE41 | session in progress, tab closed | age the round or wait 50 minutes, then /api/cron/crisis | `sweepOverrunSessions` auto-ends with reason cap; after ageing, `durationMinutes` becomes days of minutes; page "Ended automatically at the 50 minute limit." | lib/data/sessions.ts:721, lib/data/sessions.ts:754 |
...
| TE49 | in-person paid-through-us session never paid | 12 real hours later, /api/cron/reminders | `sweepInPerson` cancels it (joinTokenExpiresAt is not moved by `age`) | lib/data/in-person.ts:24 |
| TE50 | video session link | patient opens `/join/<token>` more than 12 real hours after creation | link expired (not moved by `age` because it was in the future when aged) | lib/data/sessions.ts:291 |
...
| id | setup | action | expected | code reference |
|---|---|---|---|---|
| CE1 | Clinician invitation created, then 14 real days pass | Open /clinic/join/<token> and submit | Page still renders the form (expiry not checked on render); submit says "That invitation has expired. Ask the practice for a new one." | lib/data/clinic-admin.ts:329, :408 |
...
| CE38 | Clinic manager idle 31 minutes (or after `age`) | Click anything | Redirect to /clinic/sign-in | session.ts:65 |
...
| EE14 | Coverage 80% | Lower to 50% | Pending, effective now plus 30 days; `age` never moves this future date, so within a compressed run the old 80% stays in force | sponsors.ts:779 |
...
| EE24 | Colleague invited 7 real days ago | Open the link | "That link has expired or was already used. Ask for a new one." The expiry lives inside the HMAC token, so `age` cannot move it | lib/sponsor/password-link.ts:21, sponsor-users.ts:173 |
...
| EE31 | Entries paid this week | Open /sponsor/ledger after `age` | Nothing new until the real calendar week ends: week_start is a date column that `age` does not move | lib/sponsor/ledger.ts:54, lib/db/schema.ts:7857 |
...
| ME58 | Ageing, auth | Any wave aged | Every cast session idle-expires and is purged by retention | lib/auth/session.ts:35,293-305 |
| ME59 | Ageing, invoices | Wave aged | invoices.issued_at is not aged (no created_at), so "this month" bills and invoice order ignore the shift | scripts/age.ts:178-186; lib/billing/service.ts:547-557 |
| ME60 | Ageing, pot invoices | Top-ups in several waves | Invoice numbers are counted by created_at order at read time; ageing can renumber issued invoices | lib/billing/invoice.ts:195-210 |
| ME61 | Ageing, reconcileMissingCharges | Session ended in a wave aged >= 2 days with no invoice | Never charged by the backstop | service.ts:654 |
| ME62 | Ageing, rating reminders | Wave aged > 3 days | Sessions leave the 72h window unreminded | lib/data/feedback.ts:49,797-836 |
...
| ME64 | Ageing, partner month | partner_sessions aged across a month already billed | Never billed (the month is posted once) | lib/partner/billing.ts:158-199 |
| ME65 | Ageing, list removal grace | Employee removed from the list | sponsor_email_list is not aged; the 14 days run in real time | lib/data/sponsor-email-list.ts:112-118 |
| ME66 | Ageing, deadlines | A hold, grant or link that only expires after ageing | Future deadlines are never moved; nothing expires through ageing. A deadline only passes in real time | scripts/age.ts:193-197 |
...
| id | setup | action | expected | code reference |
|---|---|---|---|---|
| ME74 | A patient covered 10% by her company books a priced session | Open the pay page | The pot pays its share at booking; the page asks for the remaining share plus VAT on that share only, and the line items say which is which | lib/billing/pot.ts, app/pay/[token]/page.tsx |
...
| ME76 | Same booking | Read the tax line | 14% of her share, never of the full price and never zero | lib/billing/employee-share.ts |
...
| ME79 | A patient opens the transfer sheet, sends the money from her bank and closes the tab without pressing Submit | Staff open `/admin/transfers` | The awaiting payment is visible to staff as an open cart with its reference, so the money can still be matched | lib/billing/manual.ts openCarts |
...
| ME86 | A patient uploaded a receipt, staff are slow, she returns an hour later | Open the pay page | Her uploaded receipt is shown back to her, not an empty upload | lib/billing/manual.ts submitProof |
| ME87 | A clinic adds a seat mid-month | Invite a clinician with no free seat | The price for the rest of the month is shown before she agrees, and exactly that figure is billed | lib/billing/seats.ts:73 |
