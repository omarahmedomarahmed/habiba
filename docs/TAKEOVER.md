...
| 11 | the rest of `docs/` | Including `docs/simulation/` |
...
`docs/DEMO-CAST.md` is generated by `npm run logins` from `scripts/_demo-cast.ts`. Twelve
...
- `docs/simulation/` is the plan for a one-month run on the live site. `verify:runbook`
  checks it against the code (commands, cron jobs, every page, every step id) but cannot
  tell you whether a screen still looks the way a step describes.
...
| `docs/DEMO-LOGINS.md` | The event logins, shareable, one password. `docs/DEMO-CAST.md` is the everyday cast. Both generated by `npm run logins` |
| `docs/simulation/` | The one-month simulation plan |
