...
| H49 | ⚠️ live | **`ship:content` is a RESEED, not an edit, and it silently replaces authored content with `defaults.ts`.** Reached for to fix three email addresses in published rows, it rewrote every page from the shipped defaults and took **6,810 bytes of live English marketing copy** with it: `for-patients` lost 41%, `pricing` 51%, and the pricing page lost its tier cards. Nothing failed at the time; `verify:sprint18` and `verify:sprint21r` went red a gate run later, naming pages rather than the cause. The published rows had been richer than `defaults.ts` since 2026-09-13 and nothing said so | Use it when the DEFAULTS are the correction (C148, H28). To change a value inside published rows, `update` those rows. Either way **measure `sum(length(blocks::text))` before and after**: same row count with fewer bytes is the signature. Recovered from a Vercel preview branch forked off production, which is a 24-hour accident and not a backup |
| H50 | ⚠️ live | **The sign-in limiter is at its production setting on a developer's machine, and re-running one verifier exhausts it.** Twenty attempts per fifteen minutes per /24. `SIMULATION_RUNNING` widens it 25x but is not set in `.env.local`, so running `verify:sprint21r` three times in a row makes it fail on checks that have nothing to do with the change under test, naming a password reset and a pricing page. It reads as a regression and is a bucket | A verifier that signs in is not idempotent within the window. Run it ONCE, and if it fails on `Too many attempts`, wait the window out rather than diagnosing the check. Same trap as the run's first rehearsal obstacle, from the other side |
| H51 | ⚠️ live | **A git worktree inside the repository is scanned as if it were the repository.** Agent worktrees live under `.claude/worktrees/`, and at least seventeen scripts walk the tree from the root skipping only `node_modules`, `.next`, `.git` and a few names. With four worktrees present, `verify:reachable` ran ten times slower and failed its 58.6 control, because a comment in one copy counted as a caller of a route in another, and `verify:sprint51` read other copies' imports | Remove every worktree (`git worktree list`, then `git worktree remove`) before running `npm run gates` or any repo-walking verifier. Never run gates while an agent is working in a worktree |
| H52 | ⚠️ live | **`db:migrate` leaked its advisory lock through the Neon pooler.** The `-pooler` endpoint is PgBouncer in transaction mode, so `pg_advisory_unlock` ran on a different server connection than `pg_try_advisory_lock`, and the lock stayed on an idle backend. The next `db:migrate` on that database printed "Another migration is already running" with nothing running. Found on dev after a Wave 2 migration; production's run of 0116 to 0124 went through the same door | `scripts/migrate.ts` connects to the direct endpoint (`-pooler` removed from the host), where one client is one session. If a leak from before the fix blocks a run, the holder is the idle backend in `pg_locks` with `objid = 24107`; ending that one backend releases it |
| H53 | ⚠️ live | **Pushing one commit to the working branch and to `main` in the same breath deploys nothing.** The Ignored Build Step (H40) skips every branch but `main`, and Vercel recorded the branch push's skipped build for that SHA; the `main` push seconds later started no build at all, so production kept the previous deployment for 25 minutes while every check said the commit was on `main`. Found in the live walkthrough when a fix that was "deployed" was not on the site | Push `main` on its own and confirm on GitHub that the commit's Vercel status says "deploying", not "Canceled by Ignored Build Step". The same SHA pushed to `main` again later does build. Confirm a deploy on the live site, never by the push alone |
| H54 | ✅ fixed | **`Promise.all` over the database ran in series.** `lib/db/index.ts` built every pool with `max: 1`, so each process had one socket and every query queued behind the one before it, including the other requests sharing an instance on Fluid compute. Parallel code read as parallel and was not: /patient/sessions took 2.6 s warm on a local production build, and making its eight money lookups a `Promise.all` changed nothing until the pool could hold more than one connection (B49) | Fixed: `POOL_MAX` in `lib/db/index.ts`. When a page is slow, time one trivial query first and divide; a page costing forty round trips is a waterfall or a queue, and `max` on the pool is the first thing to read |
