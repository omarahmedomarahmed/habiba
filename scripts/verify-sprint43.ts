 ...
 *
 * ## 🔴 AND 0079's LESSON IS A PERMANENT CHECK HERE
 *
 * 0078 left two contradictory foreign keys on one column, and my verification of it passed because
 * it read `pg_get_constraintdef` for the NAME it expected. So this file audits the whole database
 * for any column carrying more than one foreign key, by `conrelid` and `conkey`, every run. That
 * check is not about sprint 43 and it lives here because this is the sprint that had to fix it.
 */
...
async function main() {
  ...
  /*
   * 🔴 BY conrelid AND conkey, NEVER BY conname.
   *
   * 0078 dropped a constraint name that had never existed, created a fresh one, and left 0075's
   * beside it. My check read the definition of the name I expected, found `ON DELETE RESTRICT`,
   * and reported it correct: presence of the right thing without absence of the wrong one.
   */
  const doubled = await db.execute(sql`
    SELECT t.relname AS tbl, count(*)::int AS fks, string_agg(c.conname, ', ') AS names
