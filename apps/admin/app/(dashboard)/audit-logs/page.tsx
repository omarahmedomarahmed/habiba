...
const CATEGORY_COLORS: Record<AuditCategory, string> = {
  authentication: 'bg-purple-900/30 text-purple-400',
  authorization: 'bg-indigo-900/30 text-indigo-400',
  patient_data: 'bg-blue-900/30 text-blue-400',
  phi_access: 'bg-red-900/30 text-red-400',
  user_management: 'bg-amber-900/30 text-amber-400',
  org_management: 'bg-green-900/30 text-green-400',
  billing: 'bg-emerald-900/30 text-emerald-400',
  ai_operations: 'bg-cyan-900/30 text-cyan-400',
  system_config: 'bg-gray-700 text-gray-400',
  data_export: 'bg-orange-900/30 text-orange-400',
  feature_flags: 'bg-violet-900/30 text-violet-400',
  admin_actions: 'bg-rose-900/30 text-rose-400',
  security: 'bg-red-900/30 text-red-400',
};

const SEVERITY_CONFIG: Record<AuditSeverity, { color: string; dot: string }> = {
  info: { color: 'text-gray-500', dot: 'bg-gray-500' },
  low: { color: 'text-green-400', dot: 'bg-green-500' },
...
const OUTCOME_CONFIG: Record<AuditOutcome, { icon: React.ReactNode; color: string; label: string }> = {
  success: { icon: <CheckCircle2 className="w-4 h-4" />, color: 'text-green-400', label: 'Success' },
  failure: { icon: <XCircle className="w-4 h-4" />, color: 'text-red-400', label: 'Failure' },
  warning: { icon: <AlertTriangle className="w-4 h-4" />, color: 'text-amber-400', label: 'Warning' },
};
...
function RoleBadge({ role }: { role: AuditEntry['actor']['role'] }) {
  const map: Record<AuditEntry['actor']['role'], string> = {
    super_admin: 'bg-red-900/30 text-red-400',
    admin: 'bg-orange-900/30 text-orange-400',
    org_admin: 'bg-amber-900/30 text-amber-400',
    therapist: 'bg-blue-900/30 text-blue-400',
    patient: 'bg-green-900/30 text-green-400',
  ...
  return (
    <>
      ...
      >
        ...
        {/* Actor */}
        <td className="py-3 pr-4">
          <div className="text-sm text-white">{entry.actor.name}</div>
        ...
        <td className="py-3 pr-4">
          {entry.target ? (
            <div>
              <div className="text-sm text-gray-300 truncate max-w-[160px]">{entry.target.label}</div>
              <div className="text-[10px] text-gray-500 capitalize">{entry.target.type}</div>
            </div>
          ) : (
        ...
        <tr className="bg-gray-800/50 border-b border-gray-700">
          <td colSpan={7} className="px-6 pb-4 pt-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(entry.metadata).map(([k, v]) => (
                        <div key={k} className="bg-gray-800 rounded border border-gray-700 px-2 py-1">
                          <span className="text-[10px] text-gray-500">{k.replace(/_/g, ' ')}: </span>
              ...
              {/* Technical info */}
              <div className="bg-gray-900 rounded-lg border border-gray-700 p-3 space-y-2.5">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <span className="font-mono text-gray-300">{entry.ip_address}</span>
                      {entry.geo && <span className="text-gray-500"> ({entry.geo})</span>}
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                    <Cpu className="w-3.5 h-3.5 text-gray-500 mt-0.5 flex-shrink-0" />
                    <span className="text-gray-400 break-all">{entry.user_agent}</span>
                  </div>
                  {entry.session_id && (
                    <div className="flex items-center gap-2">
                      <Activity className="w-3.5 h-3.5 text-gray-500 flex-shrink-0" />
                      <span className="font-mono text-gray-400">{entry.session_id}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                    <Database className="w-3.5 h-3.5 text-gray-500 flex-shrink-0" />
                    <span className="font-mono text-gray-400">{entry.log_id}</span>
                  </div>
                  <div className="flex items-center gap-2">
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2 px-3 py-2 text-sm border border-gray-700 rounded-lg hover:bg-gray-800 transition-colors text-gray-300">
      ...
      {/* HIPAA Notice */}
      <div className="bg-blue-900/20 border border-blue-800/50 rounded-xl px-4 py-3 flex items-start gap-3">
        <Shield className="w-4 h-4 text-blue-400 mt-0.5 flex-shrink-0" />
        <div>
          <p className="text-sm font-medium text-blue-300">HIPAA Audit Trail — 6-Year Retention Policy</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard
          ...
          sub="Last 24 hours" icon={<Activity className="w-5 h-5 text-gray-400" />}
          color="bg-gray-700"
        />
        <StatCard
          label="Critical Events" value={String(stats.critical)}
          sub="Require review" icon={<AlertCircle className="w-5 h-5 text-red-400" />}
          color="bg-red-900/30"
        />
        <StatCard
          label="Failed Actions" value={String(stats.failures)}
          sub="Access denied / errors" icon={<XCircle className="w-5 h-5 text-orange-400" />}
          color="bg-orange-900/30"
        />
        <StatCard
          label="PHI Events" value={String(stats.phiEvents)}
          sub="Patient data access" icon={<FileText className="w-5 h-5 text-blue-400" />}
          color="bg-blue-900/30"
        />
        <StatCard
          label="Security Alerts" value={String(stats.securityAlerts)}
          sub="Auth & access issues" icon={<AlertTriangle className="w-5 h-5 text-amber-400" />}
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-700 p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <div className="xl:col-span-2 relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm bg-gray-800 border border-gray-700 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-500/50 placeholder-gray-500"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 text-sm bg-gray-800 border border-gray-700 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-500/50"
          >
        ...
        <div className="flex items-center justify-between mt-3">
          <p className="text-sm text-gray-400">
            ...
            <span className="font-medium text-white">{liveData.length}</span> events
          </p>
          <div className="flex items-center gap-2">
            {categoryFilter !== 'all' && (
              <button
                onClick={() => setCategoryFilter('all')}
                className="flex items-center gap-1 text-xs px-2 py-1 bg-red-900/30 text-red-400 rounded-full hover:bg-red-900/50"
              >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-800 border-b border-gray-700">
                ...
                <th className="py-3 pr-4 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">
                  Timestamp
                </th>
                ...
                <th className="py-3 pr-6 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">
                  Outcome
                </th>
              </tr>
            </thead>
          ...
          <div className="flex items-center justify-between px-6 py-3 border-t border-gray-700 bg-gray-800">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="px-3 py-1.5 text-xs border border-gray-700 text-gray-300 rounded-lg hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Previous
              </button>
                  ...
                  <button
                    ...
                    className={`w-8 h-8 text-xs rounded-lg font-medium transition-colors ${
                      page === p
                        ...
                        : 'border border-gray-700 hover:bg-gray-700 text-gray-400'
                    }`}
                  >
                    {p}
                  </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
          ...
          <div className="space-y-1.5 text-xs text-gray-400">
            <div className="flex justify-between"><span>PHI access logs</span><span className="font-medium text-white">6 years</span></div>
            <div className="flex justify-between"><span>Authentication events</span><span className="font-medium text-white">3 years</span></div>
            <div className="flex justify-between"><span>Admin actions</span><span className="font-medium text-white">6 years</span></div>
            <div className="flex justify-between"><span>System events</span><span className="font-medium text-white">1 year</span></div>
            <div className="flex justify-between"><span>Security incidents</span><span className="font-medium text-white">7 years</span></div>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {['PHI export', 'Record deletion', 'MFA disabled', 'Role escalation', 'Unauthorized access', 'API key ops', 'Impersonation'].map(t => (
              <span key={t} className="text-[10px] px-2 py-0.5 bg-amber-900/30 text-amber-400 rounded-full border border-amber-800/50">
                {t}
              </span>
            ))}
          </div>
