...
| C221 | 49 | 🔴 **REWRITTEN 2026-09-12. The premise was false: token usage IS recorded per model, and has been since before sprint 33.** The original said we cannot know what a session costs us. `ai_request_logs` carries `model`, `input_tokens`, `output_tokens`, `audio_seconds`, `cost_microcents`, `duration_ms`, `status` and `kind` across thirteen kinds, plus `organization_id`, `user_id` and `session_id`, and `logUsage` at `lib/ai/client.ts:137` writes every one of them on every call, wrapped so accounting can never take down a clinical request. `cost_microcents` exists **because somebody already hit and fixed the exact defect this row described**: `cost_cents` rounded 91% of calls to zero. **Ruling: extend the table, never add a second one.** Three additive changes deliver every sentence of the original. A **`patient_id`** column with an index, because per-patient cost (49.8) is the one thing genuinely missing. The **rate tables move into `platform_settings`** so a price can be corrected without a deploy, with `cost_microcents` still frozen at write time so a historical row keeps what it was priced at. And **"unattributable" becomes a named platform bucket in the reads**, because a null in a `GROUP BY organization_id` is silently omitted from a report, which is this plan's own defect family wearing a reporting costume. **What it costs:** one migration and a settings move, against a second writer for thirteen call kinds that would certainly have drifted. 2026-09-12. | major | **corrected by the build session, 2026-09-12** | **ruled — sprint 49** |
...
| C279 | 49 | 🔴 **The lossy cost column is already being read, and Total View would inherit it.** `ai_request_logs` has both `cost_cents` and `cost_microcents`, and `logUsage` keeps them in step. `cost_cents` is the one that rounds 91% of calls to zero, which is why microcents exists. But `lib/data/admin.ts:58`, `:382` and `:411` all sum **`cost_cents`**, while `lib/data/vault.ts:388` sums **`cost_microcents`** and divides. **Two admin surfaces already answer the same question with different numbers, today, in production.** Sprint 49 rebuilds one of them and would inherit whichever it copied from. **Ruling: every reporting path reads `cost_microcents` and nothing else, the three `admin.ts` sums are corrected as part of 49, and a verifier asserts no reporting query references `cost_cents`.** The column keeps being written for anything not yet migrated and is marked deprecated where it is defined. **What it costs:** a number on an existing admin screen will change, upwards, and somebody will ask why. The answer is that it was wrong. 2026-09-12. | major | planning review | **ruled — sprint 49** |
| C280 | 49 | **Per-patient AI cost is a behavioural log about a patient, and C244 will govern it the moment sponsors exist.** Adding `patient_id` to `ai_request_logs` makes a timestamped record of every model call concerning a person queryable by patient, which 49.8 needs and which is a new kind of row in this database. **Ruling: it is internal cost accounting and nothing else. Never patient-facing, never clinic-facing, never sponsor-facing, and inside C244's rule that no screen may join a sponsor to a patient, a session or a date.** **What it costs:** nothing today, and it is written down so that sprint 53 does not have to rediscover it. 2026-09-12. | minor | planning review | **ruled — sprints 49 and 53** |
| C281 | 41 | 🔴 **The decided sequence said pay, then the AI question, then in. The shipped code asks first, and the shipped order is right.** The spec's reason was that a consent wall after payment is pressure on somebody who has already spent money. There is no wall: declining always admits them, so the pressure being guarded against does not exist in either order. The shipped order has a documented benefit the spec did not weigh, at `app/join/[token]/actions.ts`: *a patient who agrees and then abandons Stripe has still made a decision we are obliged to honour, and one we would otherwise lose.* **Ruling: keep the shipped order. `submitJoin` is not reordered.** The build session's refusal to silently reorder a consent flow on one sentence of spec was the correct call and is now the rule: **a consent flow is never reordered on a reading of prose, only on a named harm.** **What it costs:** the plan's own sequence in §7 and sprint 41 is amended rather than the code. 2026-09-12. | major | **challenged by the build session, 2026-09-12** | **ruled — no change to code** |
| C282 | 53 | 🔴 **The AI question is bundled onto the name form, and that is the real pressure neither ordering fixes.** `submitJoin` reads `guestName` and `consent` from one `formData`. A consent answered with the momentum of filling in a name is a weaker consent than one asked on its own screen, and reordering the flow would not have touched it. The standalone `answerConsent` path already exists for a returning patient; the guest path does not use it. **Ruling: the AI question gets its own screen, always, for every path.** And one sentence sits above the choice: **"Your answer does not change whether you are seen, or what you pay."** That is what the spec's ordering was reaching for, at the cost of a line rather than a flow rewrite. **What it costs:** one more screen in the join flow, on the screen where an extra step is worth paying for. 2026-09-12. | major | planning review | **ruled — sprint 53** |
| C283 | 53 | **The couples consent rule is a read-then-write, so a grant can still land after a decline.** `recordConsent` selects the current consent, checks for `declined`, then updates. Two people answering seconds apart is safe; two answering in the same instant is not, and the rule is too important to rest on human reaction time. **Ruling: one conditional statement, no separate read.** `UPDATE ... WHERE recording_consent IS DISTINCT FROM 'declined'`, which expresses the same rule with no window and less code. **What it costs:** nothing, and the race is real however unlikely. 2026-09-12. | minor | planning review | **ruled — sprint 53** |
| C284 | 53 | 🔴 **A gate whose coverage depends on an environment variable is a gate that is quietly half open.** `verify:sprint41` branched on whether a token-encryption key was configured, testing the fail-closed refusal **or** the round trip, and printing PASS either way with a different check count nobody would read. Found and fixed inside the sprint that wrote it. **Ruling: a verifier exercises every branch of what it claims to cover in one run, whatever the machine is configured with**, and a check count that varies by environment is itself the defect. Tenth occurrence of the §6 family, and the first where the gate was half open rather than measuring the wrong thing. **What it costs:** a verifier that needs a key has to construct one rather than skip. 2026-09-12. | major | **found by the build session, 2026-09-12** | **ruled — standing rule** |
...
| C299 | 57 | 🔴 **A one-time migration left standing in a script is a loaded gun aimed at whatever the model becomes next.** `scripts/settings.ts reprice` ended with `UPDATE subscriptions SET plan='payg' WHERE plan <> 'payg'`. That was **1.7**: a single move off the old `unlimited` plan at a moment when nothing could legitimately be subscribed to. After C292 two tiers can be, and the same line would have **cancelled every paying customer's plan** — silently, from a verb an operator types to fix a price, printing a cheerful "moved to PAYG: 40 subscription(s)" underneath it. **Ruling: the sweep targets only plans the live tier table cannot price**, which is what 1.7 actually meant, and it prints the live keys beside the count. **What it costs:** nothing, and it is the second thing in this sprint that was correct when written and dangerous a year later. 2026-09-13. | blocker | **found while shipping sprint 57, 2026-09-13** | **ruled — sprint 57, fixed** |
...
| C367 | 59 | 🔴 **The case copilot's history bound was the wrong column, so a session booked for next month could reach a note written today.** `liveSince` filtered on `sessions.created_at`, which is when a row was made, not when anybody was in a room. A clinician opening the copilot on a future booking saw material from sessions that had not happened at the time the bound claimed to describe. **Ruling: bound on `started_at`, and require it to be non-null.** A session that never started has no history to contribute, and a null in a comparison is a row silently dropped rather than a row deliberately excluded. **What it costs:** a clinician preparing for a first session with somebody they have seen before gets less context than the old bound gave them, which is the correct amount. 2026-09-14. | major | review | **ruled — fixed** |
| C368 | 59 | **`cancelSession` returned void and cancelled anything.** It wrote `status = 'cancelled'` with no guard on what the status had been, so a completed session, a session already cancelled and a session in progress all cancelled identically and the caller could not tell. **Ruling: it returns a boolean and guards on `inArray(status, CANCELLABLE_FROM)`, derived from `TRANSITIONS` rather than written beside it.** A second list of legal states is a second list to forget. 2026-09-14. | major | review | **ruled — fixed** |
| C369 | 59 | 🔴 **A safety function nobody calls is a safety function that does not exist, and there were eighty-seven.** `verify:reachable` counted unreachable pages, unwired actions and uncalled routes, and did not count exported functions in the safety modules themselves. **Ruling: `uncalledExports` with a RATCHET plus a `MUST_WIRE` list of functions written for a NAMED RULING, which fail outright rather than counting against a baseline.** A baseline is for debt; a ruling with no caller is a promise that is false today. **What it costs:** the number only goes down, so every sprint that adds an export has to spend one. 2026-09-14. | blocker | review | **ruled — ratchet at 84, MUST_WIRE at 3** |
| C370 | 59 | **Muting the room did not mute the recording.** `off the record` stopped what the other person heard and the recorder kept both tracks, so the one control a patient has over what is written down changed nothing about what was written down. **Ruling: `RecorderOptions.muted`, read at construction, passed from the room's own off-record state to BOTH recorders.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C371 | 59 | **Switching plans left the old subscription running.** The new checkout opened and nothing cancelled the outgoing one, so a clinician moving from Practice to Clinic was billed for both until somebody noticed. **Ruling: the outgoing subscription is cancelled at period end BEFORE the new checkout opens, and a failure to cancel refuses the switch rather than proceeding.** Two live subscriptions is a refund conversation; a refused switch is a retry. 2026-09-14. | blocker | review | **ruled — fixed** |
| C372 | 59 | 🔴 **An export carried unapproved note content.** `lib/data/export.ts` selected `note_content` with no condition on approval, so a draft a clinician had not signed left the building inside a patient's own export. §7's first hard rule is that content in a chart needs a named clinician who approved that exact text. **Ruling: note content and its translation are gated on `status = 'approved' AND approved_at IS NOT NULL`, in the mapping rather than in the query, so an unapproved note exports as null rather than as absent.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C373 | 59 | **The copilot's bound stopped at the session list and the documents went round it.** C367 fixed the sessions read and `documentsFor` and `profileFor` kept reading everything, so the same clinician on the same future booking still saw today's uploads. **Ruling: `before` is threaded into every reader the copilot calls, not only the one that was found first.** A bound applied to one of three sources is a bound that reports the wrong thing confidently. 2026-09-14. | major | review | **ruled — fixed** |
| C374 | 59 | **`verify:claims` read page defaults and not the dictionary, which is where the words are.** Every user-facing string in this product lives in `DICTIONARIES` and the harvester walked component defaults. **Ruling: the harvester reaches both, and a CONTROL asserts it reached the dictionary specifically.** 2026-09-14. | major | review | **ruled — fixed** |
| C377 | 59 | 🔴 **The anti-differencing floor on a sponsor's pot balance was described and not built.** C229 forbids a sponsor-visible figure that moves with one person's spend, and `potBalance` returned the live balance, so an employer watching it could difference two readings and learn that somebody had a session that week. **Ruling: a published balance and a published session count, republished only when the ledger-backed count has moved by the floor.** The floor is in the data function, not in the screen, because the next screen would not have it. 2026-09-14. | blocker | review | **ruled — fixed, 0084** |
| C380 | 59 | **The application fee was computed in one currency and sent in another.** `applicationFee` was the platform cut plus settlement in cents and the charge was presented in the payer's currency, so in any non-USD jurisdiction the fee taken was the unconverted number. **Ruling: `convertAtRate(cut + settlement, quote.rateMicro)`, at the same rate the rest of the charge uses.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C381 | 59 | **A country with no implemented collection provider was offered a checkout that could not exist.** **Ruling: `collectionProblem(country)` returns a patient-readable refusal that always points at the free link, and `IMPLEMENTED_COLLECTION_PROVIDERS` is the list the code actually has rather than the list the settings screen offers.** A switch an operator can set is read by the code, or it does not exist (C218). 2026-09-14. | major | review | **ruled — fixed** |
| C382 | 59 | 🔴 **The pot debit was unconditional and ran last.** Every irreversible effect of a sponsored session happened first and the balance was then reduced with no guard, so two bookings racing spent the same money twice and an overdraft limit was advisory. **Ruling: the debit is the FIRST thing `payFromPot` claims, conditional on `balance + overdraft >= gross` in the WHERE clause, with an unconditional compensating credit if the session claim then fails.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C383 | 59 | 🔴 **A sponsored session could not be refunded at all.** The only refund path in the product required a Stripe charge, and a pot payment has none by construction. So a no-show, a cancellation or an admin correction on a sponsored session returned nothing, every time, and the automatic clock-driven refund was silently a no-op for every sponsored patient. **Ruling: `refundToPot`, reached from the same door, recovering the sponsor by walking the ledger rather than by adding a column C244 forbids.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C384 | 59 | 🔴 **C246 was written down in an index comment and made unenforceable by the hash function under it.** `enrolments_identifier_unique` is global and its own comment says "not per sponsor: an identifier that crossed one gate must not cross another", while `hashIdentifier` puts the sponsor id inside the digest, so one invented employee number at two organisations produced two different values and the index never fired. The attack C246 names was open from the day it was ruled. **Ruling: a SECOND column hashed without the sponsor, with a partial unique index, because the plaintext is deliberately never stored (C245) and existing hashes cannot be recomputed.** It reaches forward and is honest about not reaching back. **What it costs:** one identifier legitimately usable at two sponsors, such as a national ID accepted by both an employer and a university, is now refused at the second. That is what the ruling says, and it is a real person turned away. 2026-09-14. | blocker | **found by the sonnet audit, deeper than reported** | **ruled — fixed, 0085** |
| C385 | 59 | 🔴 **The check that was supposed to enforce C246 proved that a unique index is unique.** It invented one random string and inserted it twice, which is true of every unique index, and passed for the whole time the ruling was unenforced. The product never writes a hash by hand: it calls `hashIdentifier`, and that function is where the defect was. **Ruling: a check over a rule must go through the FUNCTION the product calls, with the inputs the attack uses.** Hand-crafting the value the code would have computed differently is the §6 family inside a verifier. 2026-09-14. | major | review | **ruled — fixed** |
| C386 | 59 | 🔴 **C247's promised one-step remedy had no button, so it was a zero-step remedy.** `unpause` was written when the ruling was made and had no caller for four sprints. The ruling says a paused benefit is "a real and unfair outcome" that must be "reversible by us in one step": somebody on extended leave who could not reach their work inbox had no way back at all. **Ruling: `/admin/benefits`, a queue rather than a search box, longest wait first, super_admin, audited.** A search box waits for the person to notice, work out who to ask, and ask; a queue means an operator sees every one whether or not anybody rang. 2026-09-14. | blocker | review | **ruled — built** |
| C387 | 59 | 🔴 **Two whole principals acted and nothing recorded it, while the code said otherwise.** A sponsor user ends somebody's benefit, changes the identifier gate, tops up a pot and rotates the printed joining code; a clinic manager invites a clinician, removes one and disconnects a hospital records connection. None of it wrote a row. `removeFromRoster` has taken `bySponsorUserId` since sprint 53 with the comment "for `audit`" and passed it nowhere. The cause was a good decision followed through halfway: `SponsorActor` and `ClinicActor` deliberately carry no `userId`, so they could not be passed to `audit()` either, and instead of being recorded differently they were not recorded. **Ruling: two more actor columns, the one-actor check widened from a two-way test to a count, a clinic manager beside a patient id refused outright, and the READER joined to all three or the writes are invisible.** **New gate 58.7:** every portal action file audits or is allowlisted with a reason. 2026-09-14. | blocker | review | **ruled — fixed, 0086** |
| C388 | 59 | 🔴 **A partner could write a session into the chart of a clinician it had no relationship with, and which chart was decided by arrival order.** `writeBackSession` found the clinician by email alone across the whole product, `.limit(1)`, no ordering, no partner scope, then checked the patient afterwards. `whoMayRead` twenty lines above it had been fixed for exactly this shape. Because `users_org_email_unique` is unique on (organisation, email), an address held twice made the destination arbitrary rather than merely wrong. **Ruling: the query starts at the subject's own chart, carries the same two scope lines `whoMayRead` carries, and an ambiguous address is refused with a 409 rather than resolved by whichever row Postgres returned first.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C389 | 59 | 🔴 **C277 promised a revocable link and only the grant was revocable.** `partner_subjects` had no revocation column, no function and no screen, so somebody who revoked every grant and claimed their record was still permanently that platform's "P123". It was a live capability, not a stale row: `writeBackSession` resolves the subject and asks about no grant, so sessions could keep landing in the chart of somebody who had withdrawn everything. **Ruling: `revoked_at`, filtered in `resolveSubject`, which is the single place an external reference becomes a person, so one condition closes every endpoint including the next one written.** The person's own consent screen grows the button, and the partner is told with a new `subject.unlinked` event. 2026-09-14. | blocker | review | **ruled — fixed, 0087** |
| C390 | 59 | 🔴 **Two webhook events existed, were subscribable, were asserted by a green check, and were never once emitted.** `grant.revoked` and `record.claimed` have been in `WEBHOOK_EVENTS` since 42.4 and `queueWebhook` had no caller anywhere in the product. `verify:sprint55` carried a check reading "so a partner is TOLD", which measured the enum and reported on the delivery, while `verify:reachable` had the function in `MUST_WIRE` the whole time. Two gates disagreed and the one with the reassuring sentence was the one being read. **Ruling: both emitted from the paths that cause them, the revocation scoped to the partner whose own clinician lost it, and the check rewritten to assert the emission against the code that has to run.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C391 | 59 | 🔴 **The VAT we collect had no account, so the books were short by every cent of it.** The patient's own bill says, in both languages, "VAT, paid to the government". `sessionMoney` adds the tax on top, Stripe charges it as its own line, the money clears into our balance, and `postSessionPayment` journalled `cash` at the price without it. Cash too low never trips a reconciliation that compares our own numbers to each other. A pot top-up had the mirror problem: the invoice splits out the tax and the pot was credited the whole amount, letting a sponsor spend it. **Ruling: a `vat_payable` liability account, posted on platform capture and reversed on refund, the top-up split before it reaches the pot, and all three liabilities on the vault page because a float quoted as a balance is the most flattering way to get this wrong.** 2026-09-14. | blocker | review | **ruled — fixed** |
| C392 | 59 | 🔴 **RESOLVED BY THE FOUNDER'S CURRENCY MODEL, AND THEN MADE STRUCTURAL.** The worry was that on a destination charge the clinician receives the VAT while our bill tells the patient it goes to the government. It cannot arise: *a destination charge is Stripe, Stripe is the USD rail, the USD rail is every country except Egypt, and VAT is Egypt only.* Egypt collects through its own gateway into its own entity and pays out manually. **Ruling: the invariant is a THROW in `postSessionPayment` rather than a paragraph.** A destination charge carrying tax means a country's settings disagree with the two-currency model, and the first person to set a non-zero rate on a Stripe country finds out at the moment of posting instead of at an audit. 2026-09-14. | blocker | review | **ruled — closed, guarded** |
| C393 | 59 | 🔴 **TWO CURRENCIES, AND THE COLUMN THAT DECIDES WHICH WAS FREE TEXT.** Founder, 2026-09-14: *"Only 2 currencies for now, EGP in Egypt and USD for the rest of the world. We will add GBP and EUR soon, but in those countries price with USD and pay with USD and payouts in USD."* `country_settings.currency` was a string an operator types, defaulted to `usd` and otherwise trusted, so adding the United Kingdom with `gbp` would have opened a Stripe checkout in a currency with no acquirer, no VAT rate, no payout rail and no ledger account behind it, looking correct on every screen. **Ruling: the currency is DERIVED from the entity and the two supported values are a list in `lib/billing/money.ts`.** Adding a country adds a jurisdiction, a regulator and a document list; it does not add a currency. C218's rule applied to the column next to the one it was written about. 2026-09-14. | blocker | founder | **ruled — built** |
...
| C400 | 59 | 🔴 **THE CRISIS SCANNER COULD NOT READ THE REGISTER EGYPTIANS ACTUALLY TYPE IN.** The phrase list had English, Arabic script and Egyptian dialect in Arabic script. It had nothing for **Arabizi**: Arabic in Latin letters with digits for the sounds Latin has no letter for, which is how a very large share of young Egyptians write on a phone. Somebody typing `3ayez amoot` into a check-in at 3am matched nothing at all. This is sprint 32's finding one alphabet over, in the same market, and again no number said so. **Ruling: its own fold, its own list and its own measured sensitivity.** `foldArabizi` collapses doubled letters and the three spellings of a long u; it is a SEPARATE fold because applying it to the English list would turn "better off dead" into "beter of dead". The eval reports `arz` on its own line, because folded into the Arabic figure it would hide inside a number that already looks solved. **Measured: Arabizi sensitivity 100%, specificity 83.3%, overall sensitivity 88.2% to 94.4% and specificity 76.9% to 93.1%.** ⚠️ **A seed set, named as one:** Arabizi spelling varies by person and the list is the shapes that are unambiguous, to be extended by somebody who reads the register natively. 2026-09-14. | blocker | review | **ruled — built, measured** |
...
- [x] 🔴 `settings.ts reprice` no longer sweeps **every** subscription to
      `payg`. That was 1.7's one-time move; after this sprint the same line
      cancels every paying customer, from a verb typed to fix a price (C299)
- [x] 🔴 The Stripe credit receipt stops promising a rate "yours to keep". It is
      the one string no gate in this repository can see, and that is now written
      into the file rather than left to be rediscovered (C300)
...
- [x] 🔴 **The whole sweep run, and three verifiers found to be green or red by
      accident** (C301). Each now plants what it needs: sprint 41 clones a
      session rather than hoping for a second one, sprint 49 plants a call
      costing four hundredths of a cent, sprint 16 supplies every key `lib/env`
      demands in production. All three had read as environmental for months
- [x] 🔴 `render:check`'s own control was asserting a phrase the pricing page
      stopped rendering in sprint 46, unseen for eleven sprints because the
      script needs staging rows against production and there were none (C302)
- [x] **52 verifiers, 476 checks, all green. 50/50 safety tests, 16/16 claims,
      37/37 sprint 57, `render:check` PASS against production**
...
### Sprint 69 — What the simulation needed and the product did not have · ~1 week

**Four defects, one shape.** Each one looked finished because the case that
exposes it had not happened yet, and every one of them was found by writing down
what a three-month simulation would have to do.

- [x] **69.1 · C349 — one screen, two definitions of income.** `ledgerSummary`
      counts invoices **plus** the application fees on patient payments, and says so
      in its own comment. `monthlyLedger` counted invoices alone. So the card at the
      top of `/admin/vault` and the bars below it were computed from different
      definitions of the word, and on a month carried by session fees the chart read
      near zero while the card read correctly. Both lines now, per month, split rather
      than folded, and the figures are **printed under the bars** instead of living in
      a `title=` attribute that shows nothing on a touchscreen.
- [x] **69.2 · C350 — Egypt has a crisis number.** `lib/crisis/line.ts` carried its
      own instruction for eight sprints: adding an entry is a decision somebody makes
      with a phone in their hand. On 2026-09-14 the product's owner made it. **105,
      press 1 for Arabic, then 1 for mental health**, and the two menu choices are
      part of the entry because 105 answers with a menu and nobody explores a phone
      menu in the minute that button exists for. The steps render in both languages on
      the SOS orb and on both crisis surfaces. The seed still writes null, for the
      reason `lib/settings/defs.ts` gives: stating a number twice is how two places
      come to disagree.
- [x] **69.3 · C351 — a second rejection costs something.** `submitForReview` asked
      whether every field was filled in and never whether anything had **changed**, so
      a clinician whose licence photo was rejected as unreadable could resubmit the
      identical file indefinitely. Two rejections and the documents go: the blobs are
      deleted, `documents_cleared_at` is stamped, and the resubmission is refused by
      the check that was already there rather than by a new rule. The reviewer's card
      says what the second no will do **before** it is given; the email that carries
      the reason carries the consequence; the audit line records the deletion.
- [x] **69.4 · C352 — the verification follows the clinician.** `joinWithExistingAccount`
      moved the user row and not the verification row, so a clinician who joined a
      practice left their application pointing at the practice they had left. Invisible
      for an approved clinician and not invisible for the case the path exists for: a
      **rejected** clinician invited by a practice is still rejected (C267), resubmits
      from inside it, and the operator read the wrong practice's name.
- [x] **69.5 · The copilot exam.** `npm run copilot:exam` builds questions about each
      patient out of that patient's own rows, asks the real copilot, and marks the
      answers with a grader that is shown the record's answer and the copilot's and
      nothing else. Half the questions are about things the record does **not**
      contain, because a copilot that invents a medication is failing worse than one
      that forgets a date. It reports the correlation between how much there is to
      know and how much is known, which is the memory layer's central claim as a
      number. It lives in `scripts/` rather than `evals/` because `evals/` runs against
      fixtures and this needs a populated database, which is the whole point.
- [x] **69.6 · The simulation, cut from six months to three.** 22 identities rather
      than 31, and the cut was made by merging people rather than by dropping
      scenarios. Seven documents in `docs/simulation/`, a cost estimate that says what
      to top up, and one thing **added**: a therapist rejected twice, locked out,
      invited by a practice, still refused, and approved only after reapplying.

- [x] **69.7 · C353 — `/pricing` answered 500 in production for seven sprints.** Six lines
      in `components/public/pricing-tiers.tsx` passed a function to a client component, with
      a comment above them explaining that the plural belongs where the dictionary is. The
      reasoning was right and the mechanism cannot work: React throws
      *"Functions cannot be passed directly to Client Components"* while rendering. Fixed by
      resolving one label per slider position on the server, in the shape `monthlyByCount`
      already used beside it. 🔴 **And nothing in this repository could see it.** `tsc` was
      happy, the build never rendered the page, sixty sprint verifiers read source for their
      own sprint's rules, and `verify:sprint21r` reported it red for seven sprints while
      whoever read that line explained it away as a missing published page.
- [x] **69.8 · `npm run verify:boundary`**, in `npm run gates`. No server component hands a
      function to a client one, in a prop or inside an object prop. Eleven checks, six of
      them controls, including the whole scan run over `/pricing` exactly as it stood.
- [x] **69.9 · C354 — `npm run smoke`**, also in `gates`. The stupid question nobody was
      asking: start the built app and fetch every public page in every locale. A 200 is not
      enough; the page must carry more than 200 characters of visible text, because a shell
      with an error boundary in it is a 200. **`LIVE_PAGES` was five pages old and the site
      has fourteen**, so the live check and this one were both answering honestly about a
      third of the public site. 24 page-and-locale pairs now.
- [x] **69.10 · C355 — `verify:sprint7` crashed rather than failed.** `newAccount` hardcoded
      one phone number under a unique index, so a second account in one run, or a run killed
      before its `finally`, produced a raw Postgres stack trace. Recorded for sprints as
      "stale fixture data", which describes the symptom. The number is unique per account
      now and the cleanup sweeps the reserved block, so a killed run cannot poison the next.
- [x] **69.11 · The budget, enforced rather than promised.** `npm run spend -- --budget 10`
      sums `ai_request_logs.cost_microcents`, exits non-zero past the line and warns at 70%.
      The simulation is $10 and must not stop halfway, so the orchestrator runs it after
      every wave. It also prints the per-real-session extrapolation and labels it as one.
- [x] **69.12 · `scripts/age.ts` and `scripts/simulate-seed.ts` are built**, which the
      simulation design used to ask a future session to write. `verify:age` runs the gate
      `05-AGEING.md` specifies rather than asserting it: a real row with `created_at` in the
      past and `expires_at` ten days out, aged thirty days, the first moved by exactly thirty
      and the second not at all. The seed creates the operator and the four applications and
      **nothing else**, refuses a second run, and sets the copilot quota to 4 because the run
      has $10.
- [x] **69.13 · The simulation branch is ready to work.** Migrated to 101, settings seeded,
      14 pages published, all 24 page-and-locale pairs rendering, operator signing in, four
      applications held, zero therapists, zero patients, zero sessions, $0.00 spent.
- [x] **69.14 · C356 — `check-live.ts` condemned the homepage and the pricing page on every
      run, for two reasons sprint 21R had already diagnosed and fixed next door.** It ran
      `PRICE_IN_PROSE` over rendered HTML, which cannot tell a price interpolated from
      settings a millisecond ago from one typed into a CMS body in 2025; that question moved
      to the source in sprint 46 and this copy kept asking it. And its tier-card marker was
      `/per session|session rate|EGP|USD/i`, English and hardcoded, so the **Arabic** pricing
      page failed it while rendering its cards perfectly. The marker is derived from
      `pricing.platformLine` in the reader's own dictionary now, and `PRICE_IN_PROSE` is
      deleted rather than left exported for somebody to pick up again. `npm run check:live`
      **24 pages PASS**.
- [x] **69.15 · C357 — `verify:sprint21r` crashed when the rate limiter refused it.** Five
      resets in fifteen minutes is the limiter working, and running the verifier twice trips
      it: no token is issued, five checks fail for a reason that is not a defect, and then
      `token!.id` throws. The refusal is read from the route's own words now and the block
      is deferred, loudly, because "no token" is also the genuine failure the first check
      exists to catch and the two are identical from the database.
- [x] **69.16 · Three verifiers existed on disk with no npm script.** `verify-sprint2`,
      `verify-sprint4` and `verify-sprint5` could not be run at all, which is sprint 65's
      finding one step worse: not a gate nobody runs, a gate nobody can. Wired, and all
      three pass.




### Sprint 70 — The financial model, part one: the measurement it stands on · ~1 week

**The model itself is next.** This is the half that has to exist first, because a forecast
built on a number nobody measured is a spreadsheet with a logo on it.

- [x] **70.1 · C359: a session's cost has TWO terms, and the simulation runs in the regime
      where that matters most.** `cost = FIXED + VARIABLE x minutes`. The fixed half is the
      note writer's system prompt (**1,133 tokens, counted**), the note it writes, the risk
      verdict and the profile rebuild: all of them happen once per session whatever its
      length. At three minutes the fixed half is **61%** of the bill; at fifty it is **9%**.
      🔴 **So multiplying a short session to reach a long one overstates it by 100%**:
      $0.45 against a true $0.226. At a $1 fee plus 15% that is the difference between a
      gross margin and no gross margin, and it would have been inherited by every pricing
      decision downstream.
- [x] **70.2 · `lib/finance/physics.ts`, and it fits TOKENS rather than dollars.** Two
      reasons and the second is the one that matters: dollars carry the rate table's noise on
      top of the model's, and **a price change must not invalidate a measurement.**
      Separating the physics from the prices means the day OpenAI reprices a model the whole
      forecast re-prices itself from the same rows. `tests/physics.test.ts` asserts exactly
      that: doubling the input rate moves the total and does not double it.
- [x] **70.3 · It REFUSES to fit one duration cluster.** Thirty-five sessions all the same
      length give a slope drawn through noise and an intercept equal to the mean, and the
      result is indistinguishable from a real fit by looking at it. `fit()` returns
      `{ fitted: false }` with the reason. Ten tests, every one against data whose answer is
      known by construction, because a regression returns a plausible number for every input
      and can therefore be wrong forever.
- [x] **70.4 · The simulation now produces two clusters: 24 sessions at 3 minutes, 11 at 8.**
      🔴 **It is cheaper than the flat plan it replaces**: $1.34 against $1.47, on 160 audio
      minutes against 140, because the earlier per-session estimate was high. The long ones
      are `P3`'s, who is also the top of the copilot exam's ladder, so the calibration and
      the memory test want the same thing.
- [x] **70.5 · `npm run physics`** fits per kind AND per model, because gpt-4o and
      gpt-4o-mini are priced an order of magnitude apart and blending them hides the cheap
      half. It prints the measured short session, the two-term figure for fifty minutes, and
      **the number naive multiplication would have given**, side by side.
- [x] **70.6 · C358: the operator's console is photographed in full, and C80 did not bend.**
      C80 said admin frames are never committed and its reason was disclosure of real people.
      This run has none. So the precondition is **proved rather than assumed**:
      `npm run verify:synthetic` reads every surname and every email column in the schema,
      discovered from `information_schema`, plants a real-looking name to watch itself catch
      one, and removes it in a `finally`. All 23 console pages are captured at month 3, and
      🔴 **the four `requireStaff()` pages are photographed as STAFF**, because the 24/7 team
      sees a different console from the owner and no previous capture has ever shown it.
      `.gitignore` carries the two lines to restore the moment that check stops passing.

#### What this deliberately did NOT build, and what it cannot

The model, the scenarios and `/admin/financial-model` are sprint 71. And the simulation can
only validate **half** of what goes in them, which the screen will have to say in as many
words:

| Validated by the run | Stays an assumption |
|---|---|
| Tokens and minutes per session-minute, per kind | Churn: three months, 22 people, no cohorts |
| Consent rate, which **gates** the AI fee (C209) | CAC and conversion: no marketing in the run |
| Platform fee collected against the published rate | **Stripe processing fees: test mode charges none** |
| Seat ladder billed against the published ladder | Daily video: not in the database, per participant-minute |
| The free first session | Collection lag and bad debt on the Egypt manual rail |
| Per-therapist usage **spread**, not just the mean | Salaries, hosting, everything with no proxy |

🔴 **And one figure worth stating now**, because it is the model's headline and the run will
confirm or refute it: a **fully booked** solo therapist, three patients a day for
twenty-two days, costs **$14.31 a month** in model spend at real session length, **measured**
against the live API in sprint 71. The $70 flag the founder asked for is nearly five times
that, which makes it an **anomaly detector** rather than a cost control: it fires for a clinic
sharing one login, a runaway loop, or a bug. A $99 therapist costing $14 is an **86% gross
margin before session fees**.

### Sprint 71 — The financial model itself · ~1 week

**The forecast, on a screen, with every number saying where it came from.** Sprint 70 built
the instrument; this is the thing it was built for.

- [x] **71.1 · The unit cost was MEASURED, and it corrected four of my own estimates.** Three
      runs against the live OpenAI API at 3, 8, 20 and 50 minutes, using the product's own
      `noteFromTranscript`, `classifyRisk` and `attributeLines` plus a faithful reconstruction
      of the in-session copilot. **$0.37 of spend, no database row written.** The note's system
      prompt is **995 tokens, not 1,133**; its output is **659 and SHRINKS** with length rather
      than growing; the risk verdict is **5 tokens, not 300**; and the copilot's input is
      **flat at 629** because `CONTEXT_SEGMENTS = 14` bounds it, where every earlier estimate
      had it scaling with the session. 🔴 **And the first run left out diarisation entirely**
      while the documents claimed the figure included it. Diarisation is its own `gpt-4o-mini`
      call at **243 tokens a minute**, batched at 120 lines, which is almost pure slope: the
      call that was missing was the one that grows with exactly the long sessions the business
      case rests on. Measured: `fixed = $0.01317`, `perMinute = $0.00407`, giving **$0.0254**
      at three minutes, **$0.0457** at eight and **$0.2167** at fifty, against **$0.4230** by
      multiplication, which is **95% too high**. `evals/physics.json` holds every row.
- [x] **71.2 · `lib/finance/model.ts` is one pure function and that is the design.** Thirty-six
      months, every assumption in, every month out. Changing patients-per-therapist moves
      therapists, sessions, AI cost, video cost, payment fees, revenue, gross margin, burn,
      runway and the month the cash runs out, because they are computed together rather than
      as separate formulas half of which get updated. No `Date`, no database, no `fetch`, no
      randomness, each absence asserted with a planted control, so a figure quoted in March is
      reproducible in June.
- [x] **71.3 · Every input carries `measured`, `assumed` or `derived`, with the date and the
      sample size.** "Measured on thirty-five sessions" and "measured on thirty-five thousand"
      are different claims. The provenance split prints at the TOP of the page, before any
      chart: a reader who sees the forecast first has already formed a view. Overriding a
      measured input on the screen flips it to `assumed`, whatever it was before.
- [x] **71.4 · C362: churn and the card fees can never be marked measured**, because nothing
      in this product can measure them. Stripe runs in test mode and is charged nothing; three
      months and one cohort cannot establish churn. `verify:finance` asserts it, and the amber
      card names all four unmeasurables on the screen even when no benchmark has been taken.
- [x] **71.5 · C361: growth follows the money rather than preceding it.** The first draft of
      the funded scenario applied its post-round rate from month two and **broke even in month
      four, two months before the round that paid for the salesperson who was meant to cause
      it.** `GrowthStep[]` ties a rate to the month it starts from; it now breaks even m23.
- [x] **71.6 · C363: the month it crosses zero is the alarm, and the low point is the size of
      the problem.** They are ten months apart in the base case, which **crosses zero in month
      3 and bottoms out in month 13 at $35,216 down.** Reporting only the first number lets
      somebody raise a tenth of what they need, so `deepestDeficitUsd` is on the projection,
      on the screen and in `npm run forecast`, stated as money required rather than as a
      negative balance.
- [x] **71.7 · C360: the forecast cannot move a price, and that is a property of the module
      graph.** `platform_settings` prices the product; a number in it changes somebody's next
      invoice. So the model lives in `finance_scenarios`, is read by `lib/finance/`, and
      `verify:finance` asserts that nothing under that path can import anything that writes
      money, with a control watching the scan catch a planted import. `/admin/financial-model`
      has exactly **two** write actions, both `super_admin`, both audited, and a third would
      fail the check rather than merge quietly.
- [x] **71.8 · C364: `npm run prose -- --write` was deleting the evidence three gates read.**
      Found by using it. It rebuilt `evals/prose.json` from four named fields and dropped every
      other key, which by now meant `translated.removed` — the 25 dictionary keys sprint 65
      deleted and what replaced each — that `verify:sprint65` reads directly, plus the written
      arguments for two raised floors. The documented way to record an improvement destroyed
      the record behind it, and it survived only because nobody had run `--write` since the
      block was added. That is C205's rule pointed at a WRITER: a record a tool can quietly
      drop is a record no check can rely on. Fixed by spreading the parsed file so the default
      is keep rather than drop; asserted on the source, with a control.
- [x] **71.9 · The sprint paid for its own words.** `/admin/financial-model` added **269**
      words to the admin prose sweep. The ratchet refuses a rise and `verify:sprint65` refuses
      a baseline above its origin, so both halves were paid: the screen itself was cut to 171
      words, and **fourteen genuine walls of text** were shortened across settings, the ledger
      adjuster, the partner manager, taxonomy, the therapist panel, radar, announcements, total
      view, usage, the vault and the investigate page. Admin ended at **2508 against a floor of
      2561**, DOWN by 53 while gaining a whole screen. The i18n floor went the other way,
      466→524, with the argument typed into `_i18n-coverage.json`: this screen is
      `requireRole("super_admin")` and shows salaries and runway, so it is a board pack with no
      Arabic-reading audience to acquire, and 37L.3's real debt is the operator screens the
      24/7 team uses, which this does not touch.
- [x] **71.10 · The simulation knows what it is feeding.** `07-FINANCIAL-MODEL.md` is the
      eighth document: what the run establishes, what it can never establish, how to freeze a
      measurement, and the rule that a short-session cost is never quoted as unit economics.
      `04-CAPTURE.md` photographs 24 console pages rather than 23, and the frame has to be
      taken AFTER the measurement so the provenance bar shows this database.

### Sprint 72 — The operating plan: Egypt, an angel cheque, and an offer · ~1 week

**Sprint 71 built a model. This is the plan it was built to hold.** The founder
asked what $20,000 buys, what to pay two salespeople, what marketing costs, and
whether the offer they intend to sell adds up. None of that fits an aggregate model.

- [x] **72.1 · A COHORT engine, because the offer is a function of a customer's own
      age.** One free month, then two at half price, then full: what somebody pays
      depends on how long they have been a customer, not on what month it is. An
      aggregate model has to pick one of two customers and be wrong about the other.
      `lib/finance/beta.ts` tracks every cohort's join month and reads price, churn
      and volume ramp off it. `verify:plan` proves it by moving ONE cohort's arrival
      and watching its cliff move with it: joining in month 1 pays full from month 4,
      joining in month 3 from month 6.
- [x] **72.2 · 🔴 THE CLIFF IS THE PRODUCT OF THIS SPRINT.** The month the discount
      ends is the only month that tells you anything, and it is invisible in a
      three-month beta because nobody reaches it. The model detects it by the price
      MULTIPLIER RISING rather than by counting months, so changing the offer moves
      the cliff automatically, and `verify:plan` asserts the most departures land in
      the first full-price month.
- [x] **72.3 · A volume ramp, which the first draft did not have.** An account that
      signed this month is not at full caseload this month. Without a ramp the model
      gave a month-three signature a month-three clinic, which flatters exactly the
      months a beta is judged on: month one went from 200 sessions to 73.
- [x] **72.4 · The welcome credit is CASH, the discount is not, and the model must
      not confuse them.** A discount is revenue never billed. A $200 pot credit is
      money a therapist actually withdraws, so it costs 85% of face value and is
      consumed at the rate the company's staff book. ⚠️ The first version of the check
      on this ran the plan at full price and asserted costs were unchanged; they are
      not, because removing the discount also removes the promotional churn rate.
      Two effects measured as one, which is §6. Replaced by an identity inside a
      single run plus a control that holds churn fixed.
- [x] **72.5 · The comparison that decides the pricing policy was rigged, and the
      model said so.** Grandfathering early adopters versus half price for everybody:
      the first draft gave half price BOTH better arrivals and lower churn, then
      reported that half price won. That is the assumption restated. They now differ
      in exactly one thing, what a post-beta customer pays, and half price still wins
      decisively: break even m18 against m27, $28,607 a month against $17,501.
      🔴 Because the cliff takes 25–40% of everybody while halving the price takes 50%
      of one. `verify:plan` asserts the two plans have identical arrivals, people and
      spend, so the comparison cannot silently become rigged again.
- [x] **72.6 · The headline the founder actually needed.** The three-month framing
      understates their own runway by a factor of three: **the $20,000 alone reaches
      break-even in month 10 and the cash never goes negative**, with the low point at
      $5,326 in month 9. A round becomes a choice about speed rather than survival.
- [x] **72.7 · Egypt prices, and the shipped ones are not sellable there.** $99 a
      month is ~4,950 EGP, more than many Egyptian therapists net in a week, and
      $1 + 15% + $3 on a 500 EGP session is 55% of what the patient paid. The plan
      prices in pounds at one stated rate: 1,000 EGP a therapist, 3,000 a clinic, 15%
      and no flat fee, 50 EGP for the AI. Forecast inputs only: `verify:plan` asserts
      this module cannot reach anything that bills anybody.
- [x] **72.8 · Every number says MEASURED, DECIDED or GUESS.** Two measured (the AI
      terms, imported from the benchmark rather than retyped), the rest decisions or
      guesses, each with a sentence on what it rests on. ⚠️ The check counting them
      read comment-stripped source under C205 and found zero; C205 is inverted here
      on purpose, with the reason written down, because the labels ARE the comments.
- [x] **72.9 · Video costs more than the AI. Much more.** At month 18: OpenAI $452,
      Daily $1,192, card processing $1,241. An AI company whose largest variable cost
      is a video vendor is worth knowing about before pricing anything.
- [x] **72.10 · The simulation rehearses the offer's MECHANICS, not its volumes.**
      543 sessions of the plan's month 3 would cost $117 of a $10 budget.
      `08-THE-OFFER.md` is the ninth document: the five frames that matter, the
      full-price invoice that decides everything, and the flat refusal to report a
      churn number from twenty-two people who never leave.
- [x] **72.11 · Paid for both ratchets again.** The plan screen cost 93 prose words,
      cut to 48, plus six more genuine walls shortened: admin 2508→**2497**, DOWN
      while gaining a second screen, two sprints running. i18n 524→544 with the
      argument typed into the file.

### Sprint 73 — The Egyptian rail, and it is a human being · ~1 week

**`topUpPot` refuses `entity = 'eg'`, correctly.** The cost nobody had counted is
that the entire Egyptian go-to-market then has no way to pay us: three call
centres, six clinics, nine therapists and every patient behind them.

- [x] **73.1 · The fallback rail is a bank transfer, and the details are DATA.**
      An operator types "InstaPay handle" or "Mobile wallet number" themselves, in
      their own words, in the order they want it read, with an example underneath
      and a list of who sees it. Which rails an Egyptian bank offers this quarter
      is not something a deploy should be needed to keep up with. Beside
      `egyptCollectionProvider` rather than on its own screen, because they are
      the same decision: how money reaches us from Egypt.
- [x] **73.2 · One queue for three purposes, and the action is taken on
      confirmation and never before.** A session, a subscription and a pot top-up
      are three tables; hanging "awaiting a human" off each separately gives three
      half-built states and three places to forget the rejection path. 🔴 There is
      no optimistic grant anywhere: an unconfirmed transfer is a CLAIM, and a
      product that acts on a claim about money can be robbed by typing a plausible
      reference number. The cost is a patient on a spinner, and it is paid
      deliberately.
- [x] **73.3 · Every rule that cannot be lost is a CHECK constraint.** One payer
      never two, a rejection cannot exist without a reason, a decision names who
      made it, nobody pays zero. Plus a partial unique index so four taps on a
      slow connection make one row: without it an operator confirms two and a
      session is paid for twice with no processor to reverse it.
- [x] **73.4 · The rail forces two support staff, and the plan carries them.** A
      transfer is a person checking it, by the minute, or somebody sits on a
      spinner waiting to join a therapy session. A plan that modelled the rail
      without the people would describe a product nobody can operate. $500 each,
      from month one; the founders work the same queue alongside them.
- [x] **73.5 · 🔴 THE PAYER'S SCREEN WAS WRITTEN IN ENGLISH AND THE GATE CAUGHT
      IT.** A hundred words of English on the screen where an Egyptian patient
      hands over money, against a patient floor of ZERO, which is the surface that
      floor exists to protect. Keyed in both languages rather than argued for: a
      payment screen somebody cannot read is a payment that does not happen, and
      in this market that is most of them. The rule the two halves follow is the
      same one — English is a decision for the console the founders staff, and
      never for a screen a customer pays on.
- [x] **73.6 · The therapist pitch is arithmetic they can check.** $100 unlimited
      against $4 a session is **exactly 25 sessions**. Below that, pay as you go is
      cheaper and they should use it. And the promise underneath is true rather
      than marketing: ten sessions at $20 earns $200 against a $100 bill, which
      `payouts.netFeeFromHeldEarnings` already implements.
- [x] **73.7 · Churn and growth are three named cases each, and only MEDIUM
      ships.** Good, medium, bad, with the founders' own ends: one in ten leaves
      against half. Break even moves m6 / m8 / m10 and the bad case goes $2,025
      below zero. A single number invites a reader to treat it as a forecast;
      three named ones invite the only useful question. Every deck ever built from
      a model with an optimistic toggle has used the optimistic toggle.
- [x] **73.8 · C365: `npm run smoke` reported a busy port as twenty-four dead
      pages.** Found by hitting it three times in one sprint. A leftover
      `next start` holds 3210, the new server dies on EADDRINUSE, the script talks
      to the OLD one and every request times out, and the gate prints "every
      public page in both languages timed out" — the most alarming sentence
      available, with nothing to do with the pages. ⚠️ And the first fix probed by
      FETCHING, treating a timeout as "free", which is backwards: the server that
      causes this is a hung one, bound and answering nothing. It binds now.

### Sprint 73b — The rail reaches a real payer · ~3 days

**A rail nobody can get to from a screen is a library.** This is the half that
makes it a product.

- [x] **73.9 · A receipt has its own upload kind.** `credential` is an identity
      document under a retention rule; a payment receipt should not land in the
      same bucket as somebody's licence just because both are files. 25 MB and a
      PDF allowed, because this is a photograph of a banking app taken by
      somebody who has just sent us money, and "your screenshot is 6 MB" is the
      worst possible reason for a payment not to complete.
- [x] **73.10 · The sponsor's coverage control is locked behind Edit, and says
      what the money buys.** A slider that moves on first touch is one somebody
      drags by accident on a phone, and this decides what a company pays for every
      session its staff book. Two deliberate acts, and between them the sentence
      that makes a percentage mean something: **at 10% you pay $2 of a $20
      session, so $200 covers about 100 sessions.** The average it rests on is a
      SETTING, `sponsor.averageSessionCents`, because a projection built on an
      average nobody can change is one nobody can correct.
- [x] **73.11 · The transfer details are edited beside the gateway key, and are
      LOCKED while anybody is transferring.** An operator who changes the account
      number mid-flight has sent real payments into an account we are no longer
      checking, with no processor to ask and no chargeback to raise. A refusal
      with the count in it, so the message is "four people are transferring right
      now" rather than "you cannot do that". The audit row carries the field
      LABELS and never the values.
- [x] **73.12 · 🔴 C366: `savePayouts` would have deleted the bank details.**
      `writeSettingsGroup` replaces the row, and that action built its object from
      its six form fields alone. The moment the transfer details joined the same
      group, an admin saving the netting toggle would have silently wiped the
      Egyptian account every payer's screen reads — while people were mid-transfer
      against it. **Exactly C364's shape, one file over**, found by adding a field
      to a group somebody else's form owns. Read first, then overlay: the default
      is KEEP.
- [x] **73.13 · One rail per payer, asked twice.** `sponsorNeedsTransfer` reads
      the same `entity` column `topUpPot` refuses on, so the card door and the
      transfer door can never both open. And the action re-asks it rather than
      trusting the form was rendered: a form that renders on a condition is a form
      somebody can post without meeting it.

### Sprint 73c — Three gates caught three real things · ~2 days

**Written down because all three were mine, and two were in a migration I had
already applied to production.**

- [x] **73.14 · 🔴 C367: `verify:sprint43` caught a contradiction in 0102.** The
      three payer columns were `ON DELETE SET NULL` beside a CHECK requiring
      exactly one of them non-null. Deleting a user would null the column, the
      CHECK would refuse, and the delete would fail with a constraint error naming
      a table the person deleting their account has never heard of. Same shape on
      `decided_by`: deleting the operator who confirmed a payment would break the
      row they confirmed. 🔴 **Which rule wins is the interesting part.** This
      table is the only record that money moved, with no processor to reconcile
      against, so it has to outlive the company that closed and the operator who
      left: SET NULL was right and the CHECK was wrong. `payer_kind` is the half
      that survives, so a row still says "a company paid this" after the company
      is gone.
- [x] **73.15 · And then the FIX tripped the same gate, as a false positive.**
      `(user_id IS NOT NULL)::int + … <= 1` COUNTS the columns; the detector text
      matches `<col> IS NOT NULL` and read it as a requirement. Its own comment
      says a false positive there is worth a human look, and that ruling is right
      — teaching a safety check to ignore a pattern is how a checker grows an
      exception list. So the constraint says it the way Postgres says it:
      `num_nonnulls(...) <= 1`. Same rule, shorter, and H20 is satisfied because
      the gate is green rather than explained.
- [x] **73.16 · `verify:reachable` caught an action with no screen.**
      `declarePotTransfer` was written, tested by nothing, and reachable from
      nowhere: I had built the door and not hung it. `TransferTopUp` is the form,
      and the pot page now renders exactly one rail — the card form for a sponsor
      who can be charged, the transfer form for one who cannot, never both and
      never a form that would refuse them.
...
| 2026-09-14 | 69 | **Sprint 69, what the simulation needed and the product did not have.** One screen reporting income two ways · Egypt's crisis line, eight sprints after the module asked for it · a second rejection that costs something · a verification left behind at the practice its owner walked out of · and an exam for the copilot | *sprint 69* | **Migration `0100`, additive, applied to production before this was pushed (H16) and confirmed against `information_schema`.** Two columns on `therapist_verifications`. 🔴 **All four defects are one shape: something that looked finished because the case that exposes it had not happened yet**, and all four were found by writing down what a three-month simulation would have to do rather than by a gate. **C349.** `ledgerSummary` has counted revenue as invoices PLUS application fees since Connect existed and says so in its own comment; `monthlyLedger` counted invoices alone, so the card at the top of `/admin/vault` and the bars below it answered the same question differently, and nothing said which was short. Both lines per month now, split rather than folded, netting off the settled invoice exactly as the summary nets it, and the figures are PRINTED under the bars rather than held in a `title=` attribute, which shows nothing on a touchscreen. **C350.** `lib/crisis/line.ts` spent eight sprints saying that adding a country is a decision somebody makes with a phone in their hand; on 2026-09-14 the owner made it. 105, and the two menu choices that reach the mental health service, in both languages, because a correct number with no route through it fails the same way a wrong one does. The seed still writes null. 🔴 **Three checks went red on the fact rather than on the property they protect**: `verify:sprint37r`, `verify:sprint53`'s CONTROL and `tests/crisis-line` each asserted that an Egyptian number gets NOTHING, which passed for two different reasons that looked identical: because the guard sends a reader only their own country's line, and because Egypt had no line. All three now ask the first and use a country that genuinely has none as the null half. **C351.** `submitForReview` asked whether every field was filled and never whether anything had CHANGED, so the same unreadable licence could go round the queue forever. Two rejections and the documents are deleted, blobs included; the count moves inside the UPDATE that makes the decision rather than as a number loaded a moment earlier; the refusal comes from `missingFrom`, which was already there, so there is no second rule to keep in step. The reviewer's card says what the second no will do BEFORE it is given, the email carries the consequence beside the reason, and the audit line records the deletion. Proved by rejecting a real fixture twice and reading the row, not by a regex over the function. **C352.** `joinWithExistingAccount` moved `users.organization_id` and left `therapist_verifications.organization_id` at the practice the clinician left, which is cosmetic for an approved clinician, and wrong for the case the path exists for. **69.5, the copilot exam.** `npm run copilot:exam` generates questions from a patient's own rows, asks the real copilot, and marks with a blind grader; half the questions are about things the record does NOT contain, because inventing a medication is a worse failure than forgetting a date. It reports the correlation between how much there is to know and how much is known, which is the memory layer's claim expressed as a number. In `scripts/` and not `evals/` because `evals/` runs on fixtures with no history and the history IS the thing. **69.6, the simulation cut to three months**, 22 identities from 31, merged rather than dropped, seven documents, a cost estimate ($14 of model spend, $25 to top up), and one scenario ADDED: the rejection cycle end to end. 🔴 **And the prose ratchet refused the easy way out.** The sprint added 18 words to admin and the first instinct was to record the rise with an argument, as the i18n file allows; `verify:sprint65` says a baseline may never exceed its origin, which is the stronger rule and the one that binds. So five duplications were cut to pay for it: a sentence that was on `/admin/settings` twice, a line restating its own first sentence, a policy argument that moved into the source, a subtitle describing the table beneath it, and a chart legend that repeated the column headings it sat above, and admin finished at 2561 against 2564. `verify:sprint69` **29 checks PASS**, `npm run gates` 4/4, 55 tests pass, typecheck clean, migrations level at 101 on the dev branch, production and the new simulation branch 🔴 **AND THEN IT WENT LOOKING FOR THE FAILURES IT HAD DISMISSED, AND ONE OF THEM WAS AN OUTAGE.** **C353.** `/pricing` answered 500 to every visitor IN PRODUCTION for seven sprints. Six lines in `pricing-tiers.tsx` passed a function to a client component, under a comment arguing that the plural belongs where the dictionary is; the reasoning was right and React throws on it. Fixed by resolving one label per slider position on the server, the shape `monthlyByCount` beside it already used. Nothing could see it: `tsc` was happy, the build never renders an on-demand page, sixty verifiers read their own sprint's rules, and `verify:sprint21r` DID see it and reported it red every run while the line was explained away as a missing published page. That is H20 with the volume turned down: a gate that fails for a reason somebody has explained to themselves is a gate nobody reads. **69.8, `verify:boundary`**, now in `npm run gates`: no server component hands a function to a client one, in a prop or inside an object prop, with six controls including the whole scan run over `/pricing` as it stood. **69.9, C354, `npm run smoke`**, also in gates: start the built app, fetch every public page in every locale, demand a 200 AND more than 200 characters of visible text, because an error shell is a 200. `LIVE_PAGES` was five pages old against a fourteen-page site, so the live check had been answering honestly about a third of it; 24 pairs now, all green. **69.10, C355**: `verify:sprint7` CRASHED rather than failed, because `newAccount` hardcoded one phone number under a unique index. Recorded as 'stale fixture data' for sprints, which names the symptom. Unique per account now, and the cleanup sweeps the reserved block so a killed run cannot poison the next. **69.11, the budget enforced**: `npm run spend -- --budget 10` sums `cost_microcents`, warns at 70%, exits non-zero past the line, and prints the per-real-session extrapolation labelled as one. **69.12**: `scripts/age.ts` and `scripts/simulate-seed.ts` are BUILT, which the design used to ask a future session to write, and `verify:age` runs the gate 05-AGEING.md specifies rather than asserting it. **69.13**: the simulation branch is seeded and ready, and the run was resized to 35 sessions at 4 minutes of audio with a cadence ladder (weekly, fortnightly, monthly) so $10 buys a database the copilot exam can actually separate. `verify:sprint69` 29 PASS, `verify:boundary` 11 PASS, `verify:age` 8 PASS, `verify:sprint7` PASS twice in a row, `npm run gates` 6/6, 55 tests, typecheck and build clean 🔴 **AND THEN THE SWEEP FOUND THREE MORE, ALL OF THEM GATES LYING IN ONE DIRECTION OR THE OTHER.** **C356**: `check-live.ts` condemned the homepage and the pricing page on every run, for two reasons sprint 21R had already diagnosed and fixed in its own copy. It ran `PRICE_IN_PROSE` over rendered HTML, which cannot tell a price interpolated from settings a millisecond ago from one typed into a CMS body in 2025, and that question moved to the source in sprint 46 while this copy kept asking it. Its tier-card marker was `/per session|session rate|EGP|USD/i`, English and hardcoded, so the ARABIC pricing page failed it while rendering its cards perfectly. The marker is derived from `pricing.platformLine` in the reader's own dictionary now and `PRICE_IN_PROSE` is deleted rather than left exported for the next person who needs a regex for prices. 24 live pages PASS. **C357**: `verify:sprint21r` CRASHED when the rate limiter refused it. Five resets in fifteen minutes is the limiter working as designed, and running the verifier twice trips it: no token is issued, five checks go red for a reason that is not a defect, and then `token!.id` throws over the top. The refusal is read from the route's own words now and the block defers loudly, because 'no token' is ALSO the genuine failure the first check exists to catch and the two are indistinguishable from the database. **69.16**: `verify-sprint2`, `verify-sprint4` and `verify-sprint5` existed on disk with no npm script at all, which is sprint 65's finding one step worse, not a gate nobody runs but a gate nobody CAN. Wired, and all three pass. Final state: every sprint verifier green, `npm run gates` 6/6, `npm run check:live` 24 pages, `/pricing` answering 200 in production for the first time in seven sprints |
| 2026-09-14 | 70 | **Sprint 70, the measurement the financial model will stand on.** A session's cost has two terms, not one · the console is photographed in full for the first time | *sprint 70* | **No migration.** 🔴 **C359, and it would have been inherited by every pricing decision this company makes.** `cost = FIXED + VARIABLE x minutes`. The fixed half is the note writer's system prompt, counted at **1,133 tokens**, plus the note it writes, the risk verdict and the profile rebuild, all of which happen once per session whatever its length. At three minutes that half is **61%** of the bill and at fifty it is **9%**, so the simulation runs in exactly the regime where linear extrapolation is worst: multiplying a short session to reach fifty gives **$0.45** against a true **$0.226**, an overstatement of 100%. **70.2**: `lib/finance/physics.ts` fits TOKENS rather than dollars, so a rate change re-prices the whole forecast from the same rows instead of invalidating the measurement, and `tests/physics.test.ts` asserts it by doubling the input rate and checking the total moves without doubling. **70.3**: it REFUSES a single duration cluster, because a regression through one cluster returns a slope drawn through noise and an intercept equal to the mean and looks exactly as authoritative as a real fit. Ten tests, every one against data whose answer is known by construction. **70.4**: the simulation now produces two clusters, 24 sessions at 3 minutes and 11 at 8, and it is CHEAPER than the flat plan it replaces ($1.34 against $1.47 on 160 audio minutes against 140) because the earlier per-session estimate was high. The long ones are `P3`'s, who is also the top of the copilot exam's ladder, so the calibration and the memory test want the same thing. 🔴 **70.6, C358: the operator's console is committed, and C80 did not bend.** C80 said admin frames are never committed and its reason was disclosure of REAL people; this run has none, so the precondition is proved rather than assumed. `npm run verify:synthetic` reads every surname and every email column found from `information_schema`, plants a real-looking name to watch itself catch one, and removes it in a finally. All 23 console pages at month 3, and the four `requireStaff()` pages photographed AS STAFF, because the 24/7 team sees a different console from the owner and no capture has ever shown it. `.gitignore` keeps the two lines to restore the moment that check stops passing. `test:physics` 10 PASS, `verify:synthetic` 5 PASS, every sprint verifier green, `npm run gates` 6/6 |
| 2026-09-14 | 71 | **Sprint 71, the financial model itself.** Thirty-six months, four scenarios, every number saying where it came from | *sprint 71* | **0101_financial_model** (`finance_benchmarks`, `finance_scenarios`; applied to dev, production and the simulation branch, verified against `information_schema`). 🔴 **71.1, and it corrected four of my own estimates.** Three runs against the live OpenAI API at four durations, $0.37, no database row written: the note's system prompt is **995 tokens not 1,133**, its output **659 and it SHRINKS**, the risk verdict **5 tokens not 300**, and the copilot's input **flat at 629** because `CONTEXT_SEGMENTS = 14` bounds it where every earlier estimate had it scaling. 🔴 **And the first run left diarisation out entirely** while the documents claimed it was included: it is its own `gpt-4o-mini` call at **243 tokens a minute**, almost pure slope, so the missing call was the one that grows with exactly the long sessions the business case rests on. Measured `fixed = $0.01317`, `perMinute = $0.00407`: **$0.0254** at three minutes, **$0.0457** at eight, **$0.2167** at fifty, against **$0.4230** by multiplication, **95% too high**. **71.2**: `lib/finance/model.ts` is ONE pure function, 36 months, no `Date`, no db, no `fetch`, no randomness, each absence with a planted control. **71.5, C361**: the funded scenario first applied its post-round growth from month two and broke even **two months before the round that paid for it**; `GrowthStep[]` ties a rate to its month and it now breaks even m23. **71.6, C363**: the base case crosses zero in month 3 and **bottoms out in month 13, $35,216 down** — ten months apart, so reporting only the crossing lets somebody raise a tenth of what they need; the low point is on the projection, the screen and the CLI, as money required. **71.7, C360**: the forecast cannot move a price and it is the module graph that says so, not a promise — nothing under `lib/finance/` may import anything that writes money, watched catching a planted import, and the page has exactly two write actions, both `super_admin`, both audited. 🔴 **71.8, C364, found by using it**: `npm run prose -- --write` rebuilt `evals/prose.json` from four named fields and DELETED every other key, which by now meant the 25 removals `verify:sprint65` reads and the arguments for two raised floors. The documented way to record an improvement destroyed the record behind it, unnoticed because nobody had run `--write` since the data was added. C205 aimed at a writer. **71.9**: the new screen cost admin 269 prose words, the ratchet refused the rise and `verify:sprint65` refused a raised baseline, so it was paid twice over — the screen cut to 171 and fourteen genuine walls of text shortened elsewhere, ending at **2508 against a floor of 2561**, DOWN while gaining a whole screen. `verify:finance` 30 PASS, `verify:sprint65` 42 PASS, `npm run gates` 7/7 |
| 2026-09-14 | 72 | **Sprint 72, the operating plan.** Egypt, a $20k angel cheque, an offer with a cliff in it, and the arithmetic to say what it all adds up to | *sprint 72* | **No migration.** 🔴 **A COHORT engine, because the offer is a function of the customer's own age**, not the calendar: one free month then two at half price means a therapist who joined in March and one who joined in May are billed differently in the same June, and an aggregate model has to pick one and be wrong about the other. `verify:plan` proves it by moving ONE cohort's arrival month and watching its cliff move: month 1 joiners pay full from m4, month 3 joiners from m6. **72.2, the product of the sprint**: the cliff is detected by the price multiplier RISING rather than by counting months, so changing the offer moves it automatically. **72.3**: a volume ramp the first draft lacked — without it a month-three signature got a month-three clinic and month one showed 200 sessions instead of 73. **72.4**: the $200 welcome credit is CASH at 85% of face, the discount is revenue never billed, and ⚠️ the first check on that measured two effects as one (removing the discount also removes the promotional churn rate) which is §6 again. 🔴 **72.5, AND THE MODEL CAUGHT ME RIGGING IT**: the grandfather-versus-half-price comparison gave half price both better arrivals AND lower churn, then reported half price won, which is the assumption restated. Made identical except the price a post-beta customer pays, and half price still wins decisively — break even m18 against m27 — because the cliff takes 25-40% of everybody while halving the price takes 50% of one. 🔴 **72.6, the headline**: the three-month framing understated the founders' own runway threefold. **$20,000 alone reaches break even in month 10 and never goes negative**, low point $5,326 in m9. A round is a choice about speed, not survival. **72.7**: the shipped $99 tier is ~4,950 EGP and unsellable in Egypt; the plan prices in pounds at one stated rate. **72.8**: every number labelled MEASURED, DECIDED or GUESS — two measured, and ⚠️ the check counting them read comment-stripped source under C205 and found zero, so C205 is inverted here with the reason written down. **72.9**: video costs $1,192 a month against OpenAI's $452. **72.10**: `08-THE-OFFER.md`, the ninth document — the simulation rehearses the offer's mechanics, never its volumes, and refuses to report churn from people who cannot leave. `verify:plan` 24 PASS, `npm run gates` 8/8, admin prose 2508→2497 while gaining a second screen |
| 2026-09-15 | 73 | **Sprint 73, the Egyptian rail, and it is a human being.** A bank transfer, a claim, and an operator who checks it | *sprint 73* | **0102_manual_rail** (`manual_payments`; applied to dev, production and the simulation branch, six CHECK constraints live on all three). 🔴 **`topUpPot` refuses `entity = 'eg'` and it is RIGHT to** — we cannot take a corporate card payment into an entity that does not exist yet. The cost nobody had counted is that the entire Egyptian go-to-market then had no way to pay us: three call centres, six clinics, nine therapists and every patient behind them. **73.1**: the details are DATA, typed by an operator in their own words with their own order and audience, because which rails an Egyptian bank offers this quarter is not a deploy. **73.2**: ONE queue for three purposes, and 🔴 **the action is taken on confirmation and never before** — an unconfirmed transfer is a claim, and a product that acts on a claim about money can be robbed by typing a plausible reference. **73.3**: every rule that cannot be lost is a CHECK constraint, plus a partial unique index so four taps on a slow connection make one row instead of four an operator confirms twice. **73.4**: the rail FORCES two support staff and the plan carries them, because a transfer is a person checking it while somebody sits on a spinner waiting to join a therapy session. 🔴 **73.5, AND THE GATE CAUGHT ME**: the payer's screen was written in a hundred words of English, on the surface where an Egyptian patient hands over money, against a patient floor of ZERO. Keyed in both languages rather than argued for — a payment screen somebody cannot read is a payment that does not happen. English stays a decision for the console the founders staff and never for a screen a customer pays on. **73.6**: $100 unlimited is exactly 25 PAYG sessions at $4, so the pitch is arithmetic a therapist can check, and ten sessions at $20 earns $200 against a $100 bill. **73.7**: churn and growth are three named cases and only MEDIUM ships; break even moves m6/m8/m10. 🔴 **73.8, C365**: `npm run smoke` reported a busy port as twenty-four dead pages, found by explaining it away three times in one sprint, and ⚠️ the first fix probed by FETCHING and treated a timeout as free, which is backwards for a HUNG server. It binds now. `verify:rail` 19 PASS, `verify:plan` 30 PASS, `npm run gates` 9/9 |
| 2026-09-15 | 73b | **The rail reaches a real payer.** Receipt uploads, the company's transfer door, the coverage control, and the admin field editor with its lock | *sprint 73b* | **No migration.** 🔴 **C366, and it would have deleted the bank details.** `writeSettingsGroup` replaces the row and `savePayouts` built its object from its six form fields alone, so the moment the Egyptian transfer details joined the same group an admin saving the NETTING TOGGLE would have silently wiped the account every payer's screen reads — while people were mid-transfer against it. Exactly C364's shape one file over, found by adding a field to a group somebody else's form owns. Read first, then overlay. **73.9**: a receipt gets its own upload kind, because `credential` is an identity document under a retention rule and a photograph of a banking app is not. **73.10**: the sponsor's coverage slider is locked behind Edit with Save after, and between the two acts it says the thing that makes a percentage mean anything — at 10% you pay $2 of a $20 session, so $200 covers about 100 sessions — from a SETTING rather than a constant, because a projection built on an average nobody can change is one nobody can correct. **73.11**: the transfer details are edited beside the gateway key and are LOCKED while anybody is transferring, with the count in the refusal, and the audit row carries the field labels and never the values. **73.13**: one rail per payer, asked twice — `sponsorNeedsTransfer` reads the same `entity` column `topUpPot` refuses on, and the action re-asks it rather than trusting a form was rendered. `verify:rail` 28 PASS, `npm run gates` 9/9, admin prose held level at 2476 while gaining a settings form |
| 2026-09-15 | 73c | **Three gates caught three real things, and all three were mine.** Two of them in a migration already applied to production | *sprint 73c* | **0103_payment_survives_the_payer** and **0104_num_nonnulls** (applied to dev, production and the simulation branch). 🔴 **C367**: 0102 put `ON DELETE SET NULL` on the three payer columns beside a CHECK requiring EXACTLY one non-null, and `verify:sprint43` said so the first time it ran against the new table. Deleting a user would null the column, the CHECK would refuse, and the delete would fail with a constraint error naming a table the person deleting their account has never heard of. 🔴 **Which rule wins is the point**: this table is the only record that money moved, with no processor to reconcile against, so it has to outlive the company that closed and the operator who left. SET NULL was right and the CHECK was wrong; `payer_kind` is the half that survives, so a row still says 'a company paid this' after the company is gone. **73.15**: and then the FIX tripped the same gate as a false positive, because `(user_id IS NOT NULL)::int + … <= 1` COUNTS the columns and the detector text-matches the phrase. Its own comment says a false positive there is worth a human look, and that ruling is right — teaching a safety check to ignore a pattern is how a checker grows an exception list — so the constraint now says it the way Postgres says it, `num_nonnulls(...) <= 1`. **73.16**: `verify:reachable` caught `declarePotTransfer` with no screen calling it. I had built the door and not hung it. The pot page now renders exactly one rail: the card form for a sponsor who can be charged, the transfer form for one who cannot, never both and never a form that would refuse them. `verify:rail` 28 PASS, `verify:sprint43` 49 PASS, `verify:reachable` 16 PASS, `npm run gates` 9/9 |
...
| 🔴 **A consent flow is never reordered on a reading of prose, only on a named harm.** The spec's ordering for sprint 41 was wrong and the shipped code was right (C281) | Hard |
