...
| C392 | 59 | ⚠️ **On a destination charge the clinician receives the VAT, and we tell the patient it goes to the government.** The connected account is merchant of record, the whole charge including the tax line lands in their Stripe balance, and our application fee contains none of it. If they are registered and remit it, our sentence is true. If they are not, we collected a tax on their behalf and nobody remitted it. **This is a licensing and onboarding question, not a schema one**, and it is recorded rather than decided: no code change makes a clinician VAT-registered. **What it needs:** verification status has to carry tax registration where the jurisdiction requires it, before the first non-zero-VAT country is enabled for Connect. | blocker | review | **open — needs counsel** |
