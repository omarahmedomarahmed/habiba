...
| C40 | 5 | **`ensurePersonForPatient` runs outside the join-link transaction, deliberately.** It opens its own connection, so calling it inside would deadlock against the row that transaction still holds. A patient briefly without a person is a row the next read repairs; a deadlock is a patient who cannot get into the room. The trade is recorded here because it is invisible in the code. | minor | sprint 5 | open |
