...
| C226 | 53 | 🔴 **The pot must be a payment method, not a second billing system.** The obvious build is a parallel path: corporate sessions, corporate invoices, corporate ledger accounts, a `sessionType` of `corporate`. That doubles every money code path in the product and guarantees the two drift. **Ruling: the pot stands in for the patient's card and nothing else changes.** The therapist is paid their own price, our commission comes out of it as always, VAT as always, and the therapist's own platform fee and AI fee stay theirs exactly as on any other session. One new ledger account for the pot, one new funding source at the point of payment, and no new session type. **What it costs:** a corporate session is indistinguishable from any other in every report that is not the corporate report, which is deliberate and which C242 requires anyway. 2026-09-12. | blocker | founder | **ruled — sprint 53** |
| C227 | 53 | 🔴 **The approval step hands HR a list of people who want therapy.** The founder's flow has the payer accept or reject each person, which means a named list sitting in an HR inbox against a mental health benefit. That is the precise disaster the wall exists to prevent, and it is designed in rather than leaked. **Ruling: it is safe if and only if enrolment is mass, early and framed as eligibility.** The words are **"activate your benefit"**, never "request therapy"; the drive goes to the whole workforce or student body, ideally at onboarding, before anybody needs anything; and approval is a **one-time gate after which the payer never learns whether that person booked**. A roster containing everybody carries no signal. A roster containing eleven of four hundred is a list of people in trouble. **This is a contractual obligation on the payer, not a suggestion: a client who will not run a universal enrolment drive is a client we decline.** **What it costs:** a real constraint on who we can sell to, and a sales conversation that has to explain why. 2026-09-12. | blocker | founder | **ruled — sprint 53** |
...
| C229 | 53 | 🔴 **Below a headcount, every report identifies somebody.** A five person startup with a $5,000 pot needs no analysis: any number at all is a number about one of five people. **Ruling: a floor on the roster below which the payer sees the balance and nothing else.** No counts, no trends, no heatmap, no session totals. The floor is a setting and it applies to **cohorts too**, so a university cannot slice to a nine student programme and a company cannot slice to a department. **What it costs:** our smallest clients get the least reporting, which is the opposite of what they will ask for. 2026-09-12. | major | review | **ruled — sprint 53** |
...
### 🔴 Why the approval step is safe, and the condition that makes it safe

The payer approves people. That means HR holds a list of names, and a list of
names against a therapy benefit is exactly the disaster this wall exists to
prevent. **It is safe only because of how enrolment is framed and timed.**

**Enrolment is eligibility, never therapy.** It happens before anybody needs
anything, it is pushed to the whole workforce, and the words on the screen are
**"activate your benefit"**, never "request therapy". A roster that contains
everybody carries no signal about anybody. A roster that contains eleven people
out of four hundred is a list of people in trouble.

So it is a **contractual obligation on the payer, not a nice-to-have**: the
enrolment drive goes to every employee or student, ideally at onboarding, and
the pitch is participation rather than need. A payer who will not do that is a
payer we decline.

**And the approval is a one-time gate.** Once approved, the payer never learns
whether that person booked anything, ever. Enrolling and never using it is a
normal outcome and the product must make it feel like one.
...
- [ ] **53.17** An ordinary patient signup, then **activate your benefit**:
      pick the organisation, enter the identifiers it requires, submit
- [ ] **53.18** **Pending** in the patient app, tappable to see it is under
      review. Approved gives a **badge**; rejected leaves the main view on
      dismiss and **stays in the history**
- [ ] **53.19** 🔴 A rejection reason comes from a **fixed list we control**:
      identifier not recognised, no longer with the organisation, duplicate.
      **Never free text**, because free text is where a reason nobody should
      write gets written
...
- [ ] **53.24** 🔴 **Nothing changes and nothing shows.** No therapist-facing
      query or view can reach a sponsor row, asserted by a verifier (C242)
