...
import { getPlanFeatures, PlanFeatures } from "./plan-features";
...
export class BillingService {
  ...
  /** Feature matrix for the therapist's current plan (docs/PRODUCT_MVP.md). */
  async getMyFeatures(therapistId: string): Promise<PlanFeatures> {
    const therapist = await this.db.queryOne<{ current_plan_key: string }>(
      `SELECT current_plan_key FROM therapists WHERE id = $1`,
      [therapistId],
    );
    return getPlanFeatures(therapist?.current_plan_key);
  }
