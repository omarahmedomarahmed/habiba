...
import { appendTranscriptSegment, spokenLanguagesFor } from "@/lib/data/transcript";
...
export async function POST(
  ...
  try {
    ...
    const text = await transcribeChunk({
      ...
      // Set in the room, else Arabic or English chunk by chunk when either person
      // works in Arabic (B63, board 824 / 868), else none, which means "detect it".
      // Any is better than the "en" that used to be hardcoded one layer down.
      languages: await spokenLanguagesFor(session),
    });
